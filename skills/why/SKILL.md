...
Before spawning investigators, enumerate the external data sources actually available in this session: tool servers and MCP-backed extensions, web search and fetch tools, and local stores reachable through documented commands. Use the available-tools map when present.
...
- `role`: `code` (the configured code model; no config: the parent model)
...
- `agent`: `general-purpose`
- `role`: `judgment` (the configured judgment model; no config: the parent model)
