...
- [ ] Arm the 30-minute audit tick as a watcher subagent with a 30-minute heartbeat. Never leave the cadence to memory.
...
- [ ] Apply `/no-comments` before review; sweep filler prose before each commit.
- [ ] Triage every the review bot and security-reviewer comment per `../references/bugbot-triage.md`.
...
Run each live lane in its own worktree at the PR head. Drive through the project verification harness; if none exists for a surface, drive it directly and note the gap as a risk.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes (parallel `subagent` tasks with `role: "code"`) at the PR head, per the boot recipe.
