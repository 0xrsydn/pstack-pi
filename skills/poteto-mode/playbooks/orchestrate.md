...
- **Worker / verifier.** All workers run locally on this checkout. Task types include: runtime verification through the project harness; reading this workspace's session transcripts under ~/.pi/agent/sessions/*/; simulators and local IDE state; auth that exists only here. Subagents only see their brief, so briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run restacks strictly serialized within their stack; at scale they saturate the machine.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches. Transcript mtime is not liveness. Transcript mtime is not liveness.
...
- After a machine or session restart: all running subagents are dead. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work-in-flight by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
