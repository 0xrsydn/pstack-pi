...
After the explanation is complete, spawn one architectural critic per entry of your configured `panel` role list, all in a single message. Pass `role: "panel"` on each task entry; the extension cycles the list (no config: one critic on the parent model).
