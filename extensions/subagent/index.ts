...
const ModelOverride = Type.Optional(
	Type.String({
		description:
			'Model slug for this task (e.g. "anthropic/claude-opus-4-5"). The aliases "inherit-parent" and "auto" run on the parent chat model. Overrides the agent definition.',
	}),
);

const ReadOnlyFlag = Type.Optional(
	Type.Boolean({
		description: "Restrict to read-only tools (read, grep, find, ls). Default: false.",
	}),
);
